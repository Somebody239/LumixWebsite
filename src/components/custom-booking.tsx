...
export default function CustomBooking() {
  ...
  const openSMSApp = () => {
    const businessPhone = "+12896820466"; // Business phone number
  ...
  const sendEmailNotification = async () => {
    try {
      ...
      const templateParams = {
        to_email: 'lumixwebdesign@gmail.com',
...
Please call the customer to confirm this 30-minute strategy consultation.`;
        ...
        const mailtoUrl = `mailto:lumixwebdesign@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus("idle");
      alert('There was an error submitting your booking. Please try again or call us directly at (289) 682-0466.');
    }
  };
      ...
      <Card className="w-full max-w-md mx-auto">
        <CardContent className="p-6 text-center">
          ...
          <p className="text-xs text-muted-foreground">
            Can't send? Call <strong>(289) 682-0466</strong>
          </p>
        </CardContent>
      </Card>
    );
